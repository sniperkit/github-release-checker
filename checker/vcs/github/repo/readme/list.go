...
func newListWorker(repoChan repo.Chan, readmeChan chanW) {
	for repo := range repoChan {
		...
		for _, item := range feed.Items {
			...
			readmeChan <- newTag(repo, version)
		}
	}
}

func List(repoChan repo.Chan) Chan {
	readmeChan := make(chanRW)
	...
	go func() {
		...
		for i := 0; i < maxWorkers; i++ {
			...
			go func() {
				defer wg.Done()
				newListWorker(repoChan, onlyWritable(readmeChan))
			}()
		}
	}()
