...
func newListWorker(repoChan repo.Chan, commitChan chanW) {
	for repo := range repoChan {
		...
		for _, item := range feed.Items {
			...
			commitChan <- newTag(repo, version)
		}
	}
}

func List(repoChan repo.Chan) Chan {
	commitChan := make(chanRW)
	...
	go func() {
		...
		for i := 0; i < maxWorkers; i++ {
			...
			go func() {
				defer wg.Done()
				newListWorker(repoChan, onlyWritable(commitChan))
			}()
		}
	}()
