...
func newListWorker(repoChan repo.Chan, topicChan chanW) {
	for repo := range repoChan {
		...
		for _, item := range feed.Items {
			...
			topicChan <- newTag(repo, version)
		}
	}
}

func List(repoChan repo.Chan) Chan {
	topicChan := make(chanRW)
	...
	go func() {
		...
		for i := 0; i < maxWorkers; i++ {
			...
			go func() {
				defer wg.Done()
				newListWorker(repoChan, onlyWritable(topicChan))
			}()
		}
	}()

	return onlyReadable(topicChan)
}
